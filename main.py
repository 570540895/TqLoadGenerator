...
def gen_jobs(m_duration, s_dict, s_lock):
    ...
    # create jobs according to csv file
    ...
    for index, row in df.iterrows():
        ...
        job_name = name_prefix + str(index)
        ...
        if gpu_num == 1:
            ...
            log.error('generate jobs error: index: {} gpu_num is {}.'.format(index, gpu_num))
            continue
...
def stop_jobs(m_duration, s_dict, s_lock):
    ...
    while True:
        ...
        for row in mysql_rows:
            ...
            now = int(time.time())
            if job_name not in s_dict:
                # log.error('Job: {} not found in generated jobs set while stopping job.'.format(job_name))
                continue
            stop_api_path = '/api/model/trainjobs/{}/stop'.format(uuid)
            request_url = base_url + stop_api_path
            interval = start_time+s_dict[job_name]-now if start_time+s_dict[job_name] > now else 0
            threading.Timer(interval, sendRequest.send_request, args=[request_url, 'put', headers_json]).start()
            log.info("Created a new thread to delete job {}.".format(job_name))
            with s_lock:
                del s_dict[job_name]
                log.info("Deleted Job {} in generated job set.".format(job_name))
        time.sleep(max(m_duration.value/time_compress - 1, 2))
